...
void HariMain(void)
{
    ...
    char s[40], keybuf[32], mousebuf[128], timerbuf[8], timerbuf2[8], timerbuf3[8]; /* 各種バッファ */
    ...
    for (;;) {
        ...
        if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) + fifo8_status(&timerfifo) + fifo8_status(&timerfifo2) + fifo8_status(&timerfifo3) == 0) { /* どちらからもデータが来てないことの確認 */
