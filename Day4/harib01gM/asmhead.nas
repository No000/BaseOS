...
BOTPAC  EQU     0x00280000      ; bootpackのロード先
...
[INSTRSET "i486p"]      ; 486命令まで使いたいという記述
    ...
    LGDT    [GDTR0]     ; 暫定GDTを設定
    MOV     EAX,CR0
    AND     EAX,0x7fffffff  ; bit32を0にする（ページング禁止のため）
...
memcpy:
    ...
    ADD     EDI,4
    SUB     ECX,1           ; 写経ミス
    JNZ     memcpy          ; 引き算した結果が０でなければmemcpyへ
    RET
...
GDT0:
    RESB    8
    DW      0xffff,0x0000,0x9200,0x00cf ; 読み書き可能セグメント32bit,写経ミス
