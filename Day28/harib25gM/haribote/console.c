...
void console_task(struct SHEET *sheet, int memtotal)
{
    ...
    cons.cur_y = 28;
    cons.cur_c = -1;
    task->cons = &cons; /* コンソールタスクごとに */
    task->cmdline = cmdline;    /* API用 */

    if (cons.sht != 0) {
        cons.timer = timer_alloc();
        timer_init(cons.timer, &task->fifo, 1);
        timer_settime(cons.timer, 50);   /* カーソル用（0.5秒） */
    }
...
{
    struct TASK *task = open_constask(0, memtotal);  /* 下敷きなしコンソール展開 */
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
    ...
    if (finfo != 0) {
        ...
        if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
            ...
            datsiz = *((int *) (p + 0x0010));
            dathrb = *((int *) (p + 0x0014));
            q = (char *) memman_alloc_4k(memman, segsiz);    /* APP用 */
            task->ds_base = (int) q; /* ds_baseでコードセグメントのアドレスをタスクごとにバックアップ */
            set_segmdesc(task->ldt + 0, finfo->size - 1, (int) p, AR_CODE32_ER + 0x60); /* コードセグメント */
            set_segmdesc(task->ldt + 1, segsiz - 1,      (int) q, AR_DATA32_RW + 0x60); /* データセグメント */
            for (i = 0; i < datsiz; i++) {
                q[esp + i] = p[dathrb + i];
            }
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax) /* 機能番号対応文字表示API */
{
    ...
    if (edx == 1) {
        ...
        sht = (struct SHEET *) (ebx & 0xfffffffe);
        sht->buf[sht->bxsize * edi + esi] = eax; /* 色番号 */
        if ((ebx & 1) == 0) {
        ...
        if (ecx == 0) { /* ファイルの先頭 */
            fh->pos = ebx;
        } else if (ecx == 1) {  /* 現在のｱｸｾｽ位置 */
            fh->pos += ebx;
        } else if (ecx == 2) {  /* ファイルの終端 */
            fh->pos = fh->size + ebx;
        }
        if (fh->pos < 0) {  /* 例外 */
            fh->pos = 0;
        }
        if (fh->pos > fh->size) {   /* 例外 */
            fh->pos = fh->size;
        }
        ...
        if (ecx == 0) {     /* 普通のファイルサイズ */
            reg[7] = fh->size;
        } else if (ecx == 1) {   /* 現在の読み込み位置はファイル先頭から何バイト目か */
            reg[7] = fh->pos;
        } else if (ecx == 2) {
            reg[7] = fh->pos - fh->size;
        }
    } else if (edx == 25) {     /* ファイルの読み込み */
        fh = (struct FILEHANDLE *) eax; /* ファイルハンドル */
        for (i = 0; i < ecx; i++) { /* 最大読み込みバイト数 */
            if (fh->pos == fh->size) {
                break;
            }
...
void hrb_api_linewin(struct SHEET *sht, int x0, int y0, int x1, int y1, int col)
{
    ...
    if (dx >= dy) { /* ｘ座標向きの線ベースでずらしていく */
        len = dx + 1;   /* 1を足すのは始点と終点が同じ場合の対策 */
        if (x0 > x1) {
            dx = -1024;
        } else {
            dx = 1024;
    ...
    } else {    /* y座標向きの線ベースでずらしていく */
        len = dy + 1;   /* 1を足すのは始点と終点が同じ場合の対策 */
        if (y0 > y1) {
            dy = -1024;
        } else {
            dy =  1024;
