...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
    ...
    if (finfo != 0) {
        ...
        if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
            ...
            set_segmdesc(gdt + task->sel / 8 + 1000, finfo->size - 1, (int) p, AR_CODE32_ER + 0x60); /* アプリ用のセグメント設定 */
            set_segmdesc(gdt + task->sel / 8 + 2000, segsiz - 1,      (int) q, AR_DATA32_RW + 0x60); /* たぶんアクセス権がらみ？ */
            for (i = 0; i < datsiz; i++) {
                q[esp + i] = p[dathrb + i];
            }
